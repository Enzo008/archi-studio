...
-- Features          : - Validates dependencies (Projects)
...
BEGIN
    ...
    DECLARE @V_NAME_TABLE       NVARCHAR(100) = 'TM_CLIENT',
            ...
            @V_CODE_LOG         CHAR(10) = '',
            @V_DESCRIPTION_LOG  NVARCHAR(300) = 'CLIENT DELETE',
    ...
    BEGIN TRY
        ...
        BEGIN
            SET @P_MESSAGE_TYPE = 2;
            SET @P_MESSAGE_DESCRIPTION = 'No se puede eliminar: tiene proyectos asociados';
            ROLLBACK TRANSACTION;
            RETURN;
        END
        
        UPDATE TM_CLIENT
        SET CLISTA = 'I', USEUPD = LTRIM(RTRIM(@P_USENAM_U)) + ' ' + LTRIM(RTRIM(@P_USELAS_U)),
